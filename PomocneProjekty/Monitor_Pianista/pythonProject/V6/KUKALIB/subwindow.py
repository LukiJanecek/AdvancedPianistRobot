...
def questionnaire():
    ...
    layout = [[sg.Text('Vyplňte prosím dotazník, ať vím pro koho hraji :-)', font=('Drive Book', 16)),sg.Push(),
               sg.Text('X',text_color='red',enable_events=True,font=16)],
              ...
              [sg.Checkbox("Souhlasím se zpracováním údajů", key='Souhlas', font=('Drive Book', 11), enable_events=True),
               ...
               sg.Button(button_text='Potvrdit', key='Ok', font=('Drive Book', 16),button_color='lime'),
               sg.Text('Přeskočit', font=('Drive Book', 9),enable_events=True)]]
    ...
    while True:
        ...
        if (event is not None) and ('~' in event):
            key_value = event.split('~')
            data[key_value[0]] = key_value[1]

        if event == 'Odkaz':
            sg.popup_no_buttons('\n'*15+'Zde bude PDF nebo PNG dokumentu upřesňujícího způsob zpracování a využívání získaných '
                        'odobních dat katedrou'+'\n'*15, keep_on_top=True, title='GDPR',background_color='white')
