    ...
    while (running) {
		...
		lastInputTime.store(steady_clock::now());
                ...
                else if (value>=4 && value<=LED_COUNT-1)
                {
                    ...
                    // === VRï¿½CENO ï¿½ pï¿½vodnï¿½ nï¿½hodnï¿½ rozsah 0ï¿½255 ===
...
void effect_special3() {
    static uint64_t t0 = millis();
    float t = (millis() - t0) * 0.0025f; // ovládá rychlost vln

    for (int i = 0; i < LED_COUNT; i++) {
        // jemné pulzování celého pásku
        float wave = (sin(t + i * 0.05f) + 1.0f) * 0.5f; // 0..1
        uint8_t val = (uint8_t)(150 + wave * 100);       // 150..250 brightness
        uint8_t sat = 60;                                // elegantní nízká saturace
        uint8_t hue = 30;                                // zlatý tón

        ledstring.channel[0].leds[i] = CHSV_to_RGB(hue, sat, val);
    }

    // náhodné jemné jiskry
    if ((rand() & 0xFF) < 6) { // cca 2-3% šance každý frame
        int p = rand() % LED_COUNT;
        uint8_t sparkleVal = 255;
        uint8_t sparkleSat = 20;
        uint8_t sparkleHue = 40 + (rand() % 10); // svìtlejší zlato/bílá
...
effect_fn effects[] = {
    effect_toceniLedekbarva_paleta,	// upravit paletu na zajimavou
    effect_prolinani,				// basic duha
	effect_dualWaveCollision,		//
	effect_confetti,				//
	effect_aurora,					//
    effect_rainbow_with_glitter,	//
    effect_kometa,					// absolutnÄ› nedÄ›lÃ¡ co by mÄ›la
    effect_bpm,						//
